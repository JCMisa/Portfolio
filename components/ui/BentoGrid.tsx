...
export const BentoGridItem = ({
  ...
  const rightLists = [
    ...
    { name: "NextJS", icon: <MdTrendingUp />, status: "up" },
    { name: "SQL", icon: <MdTrendingFlat />, status: "flat" },
  ];
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div
          className={cn(
            titleClassName,
            "group-hover/bento:scale-[0.9] transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10"
          )}
        >
              ...
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-4">
                {leftLists.map((item, i) => (
                  <span
                    ...
                    lg:opacity-100 rounded-lg text-center bg-[#10132E] flex flex-row gap-2"
                  >
                    <p
                      className={`
                        ${item.status === "up" && "text-green-500"}
                        ${item.status === "flat" && "text-yellow-500"}
